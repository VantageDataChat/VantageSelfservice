...
func TestInitDB_CreatesTablesSuccessfully(t *testing.T) {
	...
	for _, table := range expectedTables {
		var name string
		err := dbPair.Write.QueryRow("SELECT name FROM sqlite_master WHERE type='table' AND name=?", table).Scan(&name)
		if err != nil {
...
func TestInitDB_WALModeEnabled(t *testing.T) {
	...
	if err := dbPair.Write.QueryRow("PRAGMA journal_mode").Scan(&journalMode); err != nil {
...
func TestInitDB_ForeignKeysEnabled(t *testing.T) {
	...
	var fk int
	if err := dbPair.Write.QueryRow("PRAGMA foreign_keys").Scan(&fk); err != nil {
...
func TestInitDB_ForeignKeyEnforcement(t *testing.T) {
	...
	_, err = dbPair.Write.Exec("INSERT INTO sessions (id, user_id, expires_at) VALUES ('s1', 'nonexistent', '2099-01-01')")
	if err == nil {
